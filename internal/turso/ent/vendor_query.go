...
func (_q *VendorQuery) QueryPackages() *MediaPackageQuery {
...
func (_q *VendorQuery) FirstID(ctx context.Context) (id string, err error) {
	var ids []string
...
func (_q *VendorQuery) FirstIDX(ctx context.Context) string {
...
func (_q *VendorQuery) OnlyID(ctx context.Context) (id string, err error) {
	var ids []string
...
func (_q *VendorQuery) OnlyIDX(ctx context.Context) string {
...
func (_q *VendorQuery) IDs(ctx context.Context) (ids []string, err error) {
...
func (_q *VendorQuery) IDsX(ctx context.Context) []string {
	ids, err := _q.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (_q *VendorQuery) WithPackages(opts ...func(*MediaPackageQuery)) *VendorQuery {
	query := (&MediaPackageClient{config: _q.config}).Query()
...
func (_q *VendorQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Vendor, error) {
	...
	if query := _q.withPackages; query != nil {
		if err := _q.loadPackages(ctx, query, nodes,
			func(n *Vendor) { n.Edges.Packages = []*MediaPackage{} },
			func(n *Vendor, e *MediaPackage) { n.Edges.Packages = append(n.Edges.Packages, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *VendorQuery) loadPackages(ctx context.Context, query *MediaPackageQuery, nodes []*Vendor, init func(*Vendor), assign func(*Vendor, *MediaPackage)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*Vendor)
	for i := range nodes {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(mediapackage.FieldVendorID)
	}
	query.Where(predicate.MediaPackage(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(vendor.PackagesColumn), fks...))
	}))
...
func (_q *VendorQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(vendor.Table, vendor.Columns, sqlgraph.NewFieldSpec(vendor.FieldID, field.TypeString))
	_spec.From = _q.sql
