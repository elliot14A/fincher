...
func (_q *RunQuery) QuerySteps() *StepQuery {
...
func (_q *RunQuery) QueryResults() *WfResultQuery {
...
func (_q *RunQuery) WithSteps(opts ...func(*StepQuery)) *RunQuery {
	query := (&StepClient{config: _q.config}).Query()
...
func (_q *RunQuery) WithResults(opts ...func(*WfResultQuery)) *RunQuery {
	query := (&WfResultClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *RunQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Run, error) {
	...
	if query := _q.withSteps; query != nil {
		if err := _q.loadSteps(ctx, query, nodes,
			func(n *Run) { n.Edges.Steps = []*Step{} },
			func(n *Run, e *Step) { n.Edges.Steps = append(n.Edges.Steps, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withResults; query != nil {
		if err := _q.loadResults(ctx, query, nodes,
			func(n *Run) { n.Edges.Results = []*WfResult{} },
			func(n *Run, e *WfResult) { n.Edges.Results = append(n.Edges.Results, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *RunQuery) loadSteps(ctx context.Context, query *StepQuery, nodes []*Run, init func(*Run), assign func(*Run, *Step)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(step.FieldRunID)
	}
...
func (_q *RunQuery) loadResults(ctx context.Context, query *WfResultQuery, nodes []*Run, init func(*Run), assign func(*Run, *WfResult)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*Run)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(wfresult.FieldRunID)
	}
	query.Where(predicate.WfResult(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(run.ResultsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
