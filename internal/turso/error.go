...
func NewError(op string, code domainerrors.ErrorCode, message string, cause error) *domainerrors.DomainError {
	...
	if code == domainerrors.CodeInternal {
		logger.Error("turso store internal error", "op", op, "code", code, "message", message, "cause", cause)
	} else {
		logger.Warn("turso store client error", "op", op, "code", code, "message", message, "cause", cause)
	}
	return domErr
}
...
func MapEntError(op, entity, id string, err error) *domainerrors.DomainError {
	...
	if ent.IsNotFound(err) {
		...
		logger.Warn("record not found", "op", op, "entity", entity, "id", id)
		return domErr
	}
	...
	if ent.IsConstraintError(err) {
		...
		logger.Warn("constraint error", "op", op, "entity", entity, "id", id, "code", domErr.Code, "cause", err)
		return domErr
	}
	...
	logger.Error("database failure", "op", op, "entity", entity, "id", id, "cause", err)
	return domErr
}
