...
describe('calculateCountdown', () => {
  ...
  it('calculates days, hours, minutes, seconds for future dates', () => {
    const now = 1700000000000
    ...
    expect(result.timecode).toBe('02h 15m 30s')
    expect(result.label).toBe('2h 15m left')
  })

  it('formats multi-day range with days, hours, mins, secs', () => {
    const now = 1700000000000
    // 3 days, 4 hours, 12 minutes, 5 seconds
    ...
    expect(result.label).toBe('3d 4h left')
    expect(result.timecode).toBe('3d 04h 12m 05s')
  })
})
