...
* **Ent Schemas & Code Generation**:
  * `internal/turso/ent/schema/master.go`, `vendor.go`, `media_package.go` with strict FK edges (`Title 1──* Master`, `Title 1──* Package`, `Vendor 1──* Package`).
* **Turso Actions**:
  * Pure functional actions in `internal/turso/masters/`, `packages/`, `vendors/` returning domain models.
